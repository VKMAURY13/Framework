...
public class Organization_Static_test_Test extends BaseClass{
	
	@Test
	public void createOrganizationwithOrgName() throws EncryptedDocumentException, IOException
	{
		//Fetch Data From Excel Sheet
		ListenerImpClass.test.log(Status.INFO, "Fetch Data From Excel Sheet");
		String ORGNAME =  elib.getDataFromExcel("Organization", 1, 2)+jlib.getRandomNumber();
		
		//click on Organization Link
		ListenerImpClass.test.log(Status.INFO, "click on Organization Link");
		HomePage hp = new HomePage(driver);
		hp.getOrgmoduletab().click();
		
		//click on lookup Icon
		ListenerImpClass.test.log(Status.INFO, "click on lookup Icon");
		OrganizationsPage op = new OrganizationsPage(driver);
		op.getCreateneworgbtn().click();
				
		//Insert Organization name
		ListenerImpClass.test.log(Status.INFO, "Insert Organization name");
		CreatingNewOrganizationPage cnop = new CreatingNewOrganizationPage(driver);
		cnop.getOrganizationname().sendKeys(ORGNAME);
		
		//click on save button
		ListenerImpClass.test.log(Status.INFO, "click on save button");
		cnop.getSavebtn().click();
		
		//verification by HeaderText
		ListenerImpClass.test.log(Status.INFO, "verification by HeaderText");
		...
		ListenerImpClass.test.log(Status.INFO, "Verification by Organization Name text");
